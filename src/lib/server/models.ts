import { HF_ACCESS_TOKEN, MODELS, SUMMARIZE_MODELS, OLD_MODELS } from "$env/static/private";
import { z } from "zod";
...
const modelsSummarize = z
	...
	.parse(JSON.parse(SUMMARIZE_MODELS));

export const models_summarize = await Promise.all(
	modelsSummarize.map(async (m) => ({
		...m,
		id: m.id || m.name,
		displayName: m.displayName || m.name,
		preprompt: m.prepromptUrl ? await fetch(m.prepromptUrl).then((r) => r.text()) : m.preprompt,
...
export const defaultsummarizeModel = models_summarize[0];
